...
# sophia's edits:
# Removed hnswlib import to unify all ANN implementations under FAISS.
# Reason: Ensures architectural consistency, fair benchmarking, and cleaner dependency stack.
...
def batch_recall(test_I, truth_I, k):
    # Computes average recall@k over all queries
    results = []
    for i, row in enumerate(test_I):
        results.append(recall_at_k(row, truth_I[i], k))
    return sum(results) / len(results)
...
def brute_force_build():
    """
    Builds exact L2 index using FAISS IndexFlatL2.
    Used as ground truth reference for recall measurement.
    """
    global FL2
    FL2 = faiss.IndexFlatL2(d)
    FL2.add(x_train)
...
def brute_force_search(k, measure_accuracy=False):
    ...
    Stores ground truth neighbors for recall comparison.
    """
    global FL2, truth_D, truth_I
    truth_D, truth_I = FL2.search(x_query, k)
    if measure_accuracy:
...
def search(index, k, measure_accuracy=True):
    """
    Wrapper to run search on FAISS index and compute recall.
    Keeps evaluation logic unified across index types.
    """
    D, I = index.search(x_query, k)
    if measure_accuracy:
        global truth_I
...
def lsh_build(n_bits):
    """
    Builds FAISS LSH index.
    Used to evaluate hash-based ANN performance.
    """
    global LSH
    LSH = faiss.IndexLSH(d, n_bits)
    LSH.add(x_train)
...
def pq_build(subquantizers, n_bits):
    """
    Builds FAISS PQ index.
    Used to evaluate compressed vector search trade-offs.
    """
    global PQ
    PQ = faiss.IndexPQ(d, subquantizers, n_bits)
    PQ.train(x_train)
...
def ivfpq_build(ncentroids, code_size, n_bits):
    """
    Builds IVF-PQ index.
    Combines coarse quantization (IVF) and compression (PQ)
    for scalable approximate search.
    """
    global IVFPQ
    coarse_quantizer = faiss.IndexFlatL2(d)
    IVFPQ = faiss.IndexIVFPQ(coarse_quantizer, d, ncentroids, code_size, n_bits)
    IVFPQ.train(x_train)
    IVFPQ.add(x_train)
    IVFPQ.nprobe = 5  # Controls search breadth inside clusters


# ==============================
# sophia's edits:
# Replaced hnswlib with FAISS HNSW implementation
# Reason:
# - Ensures uniform backend (FAISS-only)
# - Fair performance comparison
# - Avoids cross-library overhead differences
# ==============================

def hnsw_build(M=32, ef_construction=200, ef_search=100):
    """
    Builds FAISS HNSW index (graph-based ANN).
    M controls graph connectivity.
    efConstruction controls build accuracy.
    efSearch controls query accuracy/speed trade-off.
    """
    global HNSW
...
def hnsw_search(k, measure_accuracy=True):
    """
    Executes HNSW search and computes recall.
    """
    global HNSW, truth_I
...
def test_build(mem=None):
    """
    Builds all index types sequentially.
    Optional memory monitoring support for research logging.
    """

    if mem: mem.log("before_brute_force")
    brute_force_build()
    if mem: mem.log("after_brute_force")

    if mem: mem.log("before_lsh")
    lsh_build(lsh_nbits)
    if mem: mem.log("after_lsh")

    if mem: mem.log("before_pq")
    pq_build(pq_subquantizers, pq_nbits)
    if mem: mem.log("after_pq")

    if mem: mem.log("before_ivfpq")
    ivfpq_build(ivfpq_ncentroids, ivfpq_codesize, pq_nbits)
    if mem: mem.log("after_ivfpq")

    if mem: mem.log("before_hnsw")
    hnsw_build(
        M=HNSW_M,
        ef_construction=HNSW_efconstruction,
        ef_search=HNSW_efsearch
    )
    if mem: mem.log("after_hnsw")
...
if __name__ == "__main__":
    ...
    if d in dim_to_subspaces:
        ...
        raise ValueError("Dimension not supported for PQ configuration.")
    ...
    x_query = df.iloc[test_i].to_numpy(dtype=np.float32, copy=False)
    x_train = df.iloc[train_i].to_numpy(dtype=np.float32, copy=False)

    print("Train Size:", x_train.shape[0])
    print("Test  Size:", x_query.shape[0])
    ...
    if not args.no_save:
        ...
        faiss.write_index(PQ,    "indices/pq.index")
        faiss.write_index(IVFPQ, "indices/ivfpq.index")
        faiss.write_index(HNSW,  "indices/hnsw.index")

        np.save("indices/x_query.npy", x_query)
        np.save("indices/x_train.npy", x_train)
