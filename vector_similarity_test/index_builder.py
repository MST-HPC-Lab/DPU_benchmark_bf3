...
def hnsw_build(data, dim, ef_construction=200, M=16, ef_search=100):
    ...
    HNSW.add(data)  # expects contiguous float32 numpy
...
def test_build(mem=None):
    ...
    # OLD (BUGGY) CALL SITE (COMMENTED OUT — DO NOT DELETE)
    # It used x_train.to_numpy() which (in your old code) was often float64
    # and sometimes non-contiguous depending on pandas view behavior.
    # hnsw_build(x_train.to_numpy(), d, HNSW_efconstruction, HNSW_M, HNSW_efsearch)

    # SOPHIA EDIT: Pass the correct contiguous float32 training matrix
...
if __name__ == "__main__":
    ...
    # SOPHIA EDIT: FAISS expects contiguous float32 numpy arrays.
    # Convert ONCE here so all builders/searchers work reliably.
    ...
    if not args.no_save:
        ...
        # SOPHIA EDIT: Save FAISS HNSW using faiss.write_index
