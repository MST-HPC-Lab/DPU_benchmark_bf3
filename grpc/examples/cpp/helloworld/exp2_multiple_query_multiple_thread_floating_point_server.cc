...
map<std::string, GEOSGeometry*> get_polygon_map_without_id(const char* filename,
                                                GEOSContextHandle_t ctx) {
  map<std::string, GEOSGeometry*> geom_map;
  std::ifstream input(filename);
  int ai_id = 0;
...
class ServerImpl final {
 public:
  ...
  void Run() {
    ...
    // const char* filename = "/home/dpu/data/cemet_data_indexed";
    const char* filename = "/global/scratch/users/satishp/data/cemet_data_indexed";

    map<std::string, GEOSGeometry*> geoms = get_polygon_map(filename, ctx);
    // const char* filename = "/home/dpu/data/lakes_data";
    // map<std::string, GEOSGeometry*> geoms = get_polygon_map_without_id(filename, ctx);
