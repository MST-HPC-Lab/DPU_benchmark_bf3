...
class ServerImpl final {
 public:
  ...
  void Run() {
    ...
    const char* filename = "/home/ubuntu/data/cemet_data_indexed";
  ...
  static void sendWork(string cpuQueryString) {
    GreeterClient greeter(grpc::CreateChannel(
        "192.168.100.1:50051", grpc::InsecureChannelCredentials()));
    cout << "Sending to cpu..." << endl;
    std::string user(cpuQueryString);
    cpuReply = greeter.SayHello(user);
    std::cout << "Greeter received from CPU: " << cpuReply << std::endl;
  }
...
//       // geos::geom::LinearRing* ring = new geos::geom::LinearRing(coords_arr,
//       // gf); std::unique_ptr<geos::geom::Polygon> poly(gf->createPolygon(ring,
//       // nullptr));
...
// #ifdef DEBUG
//     for (ulong k = 0; k < 2 * numVertices; ++k) printf("%3.16f ", geomArray[k]);
