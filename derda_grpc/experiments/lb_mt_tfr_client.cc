...
class GreeterClient {
 public:
  ...
  std::string SayHello(const std::string& user) {
    ...
    float points[] = {5.43, 3.44};
    for (size_t i = 0; i < 2; i++)
    {
      request.add_geom(points[i]);
    }
...
void doWork(GEOSContextHandle_t ctx) {
  while (!jobQueue.empty()) {
    ...
    start_test("/home/dpu/data/" + work, "/home/dpu/data/cemet_data_indexed",
               ctx);
...
//   //while (!jobQueue.empty()) { // todo: update for multiple files
//   if (!jobQueue.empty()) {
...
void intersect_callback(void* geom, void* base_geom) {
  if (!GEOSIntersects(static_cast<const GEOSGeometry*>(geom),
    ...
    char* wkt_geom =
        GEOSWKTWriter_write(writer, static_cast<const GEOSGeometry*>(geom));
    char* wkt_base_geom = GEOSWKTWriter_write(
        writer, static_cast<const GEOSGeometry*>(base_geom));
    GEOSWKTWriter_destroy(writer);
    cout << "Intersect: " << wkt_base_geom << " - " << wkt_geom << endl;
  }
}

void intersectsRefinement() {
  GreeterClient greeter(grpc::CreateChannel(
      "192.168.100.1:50051", grpc::InsecureChannelCredentials()));

  std::string reply = greeter.SayHello("next");  // The actual RPC call!
  std::cout << "Greeter received: " << reply << std::endl;

  while (reply != "done") {
    string base_geom_string = reply.substr(0, reply.find(" - "));
    string query_geom_string = reply.substr(reply.find(" - ") + 3);
    GEOSGeometry* base_geom = GEOSGeomFromWKT(base_geom_string.c_str());
    GEOSGeometry* query_geom = GEOSGeomFromWKT(query_geom_string.c_str());
    intersect_callback(base_geom, query_geom);

    reply = greeter.SayHello("next");  // The actual RPC call!
    std::cout << "Greeter received: " << reply << std::endl;
  }
...
int main(int argc, char** argv) {
  ...
  // jobQueue.push("/global/scratch/users/satishp/data/sports_data_indexed");
  //  jobQueue.push("/global/scratch/users/satishp/data/sports_indexed_128/0");
  // jobQueue.push("sports_indexed_128/0");
  ...
  for (std::string line; getline(input, line);) {
    if (line.length() > 5 && line.find("(")) {
      ...
      // cout << "Sending " << geom_string << " ..." << endl;
      std::string reply = greeter.SayHello(geom_string);
      if (reply != "No intersection") {
        std::cout << "Result: " << geom_string << " - " << reply << std::endl;
      }
    }
  }

  // string geom_string =
  //     "POLYGON ((9.1170569 53.456449, 9.1183306 53.4562948, 9.1184809 "
  //     "53.4566717, 9.1171457 53.4568074, 9.1170569 53.456449))";

  // cout << "Sending " << geom_string << " ..." << endl;

  // std::string reply = greeter.SayHello(geom_string);  // The actual RPC call!
...
int start_test(std::string file1, std::string file2, GEOSContextHandle_t ctx) {
  ...
  map<std::string, GEOSGeometry*> geom_map = get_polygon_map(filename, ctx);

  cout << geom_map["1"] << endl;
  cout << geom_map["10"] << endl;
  cout << geom_map["34"] << endl;

  // // Get an iterator pointing to the first element in the map
  // std::map<std::string, int>::iterator it = geom_map.begin();

  // // Iterate through the map and print the elements
  // while (it != geom_map.end())
  // {
