...
class GreeterClient {
 public:
  ...
  // from the server.
  std::string SayHello(const std::string& filename1) {
    // Data we are sending to the server.
    HelloRequest request;
    request.set_name(filename1);
    ...
    start_test("uniform-10k.wkt", "uniform-5.wkt");
...
int main(int argc, char** argv) {
  ...
  std::string file1("uniform-10k.wkt");
  std::string reply = greeter.SayHello(file1);
